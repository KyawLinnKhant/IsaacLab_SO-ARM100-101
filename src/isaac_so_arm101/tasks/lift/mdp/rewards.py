...
def gripper_obj_dist(
    ...
    obj_world_pos = object.data.root_pos_w
    # End-effector position: (num_envs, 3)
    gripper_world_pos = ee_frame.data.target_pos_w[..., 0, :]
    # Distance of the end-effector to the object: (num_envs,)
    gripper_obj_dist = torch.norm(obj_world_pos - gripper_world_pos, dim=1)

    return 1 - torch.tanh(gripper_obj_dist / std)
...
def object_goal_distance(
    ...
    return (object.data.root_pos_w[:, 2] > minimal_height) * (1 - torch.tanh(goal_dist / std))
...
def object_ee_distance_and_lifted(
    ...
    # Get reaching reward
    approach_reward = gripper_obj_dist(env, std, object_cfg, ee_frame_cfg)
    # Get lifting reward
    elevation_reward = object_is_lifted(env, minimal_height, object_cfg)
    # Combine rewards multiplicatively
    return approach_reward * elevation_reward
