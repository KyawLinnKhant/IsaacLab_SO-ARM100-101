...
class SensorObsCfg:
    ...
    @configclass
    class AgentObsCfg(ObsGroup):
        """Observations for inference_policy group."""

        joint_pos = ObsTerm(func=mdp.joint_pos_rel)
        joint_vel = ObsTerm(func=mdp.joint_vel_rel)
        object_position = ObsTerm(func=mdp.object_position_in_robot_root_frame)
        target_object_position = ObsTerm(func=mdp.generated_commands, params={"command_name": "object_pose"})
        joint_actions = ObsTerm(func=mdp.last_action)
    ...
    # observation groups
    inference_policy: AgentObsCfg = AgentObsCfg()
...
class TrainingRewardsCfg:
    ...
    reaching_object = RewTerm(func=mdp.gripper_obj_dist, params={"std": 0.05}, weight=1.0)
...
class CubeLiftEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Scene settings
    scene: ManipulationSceneCfg = ManipulationSceneCfg(num_envs=4096, env_spacing=2.5)
    # Basic settings
    observations: SensorObsCfg = SensorObsCfg()
    joint_actions: JointActionsCfg = JointActionsCfg()
    commands: MotionCommandsCfg = MotionCommandsCfg()
    # MDP settings
    rewards: TrainingRewardsCfg = TrainingRewardsCfg()
    terminations: EpisodeDoneCfg = EpisodeDoneCfg()
    events: SimEventsCfg = SimEventsCfg()
    curriculum: LearningCurriculumCfg = LearningCurriculumCfg()
    ...
    def __post_init__(self):
        ...
        # simulation settings
        self.sim.sim_step_dt = 0.01  # 100Hz
